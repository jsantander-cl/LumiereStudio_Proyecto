...
function Footer() {
  ...
  return (
    <footer id="contacto" className="bg-[var(--color-abrow-footer)] pt-16 pb-8 px-6 md:px-12">
      <div className="max-w-7xl mx-auto space-y-14">
        {/* Logotipo Central */}
        <div className="text-center">
          <h4 className="font-serif text-3xl text-[var(--color-abrow-dark)]">Lumiere</h4>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-12 text-sm">
          ...
          <div className="space-y-3">
            <h5 className="font-serif text-lg text-[var(--color-abrow-dark)]">Horario de atención</h5>
          ...
          <div className="flex flex-col items-center text-center space-y-6">
            <p className="font-sans text-sm text-[var(--color-abrow-muted)] max-w-xs">
              Somos un acogedor estudio de belleza en la ciudad de Antofagasta. Transformamos tu imagen cambiando tu mirada.
            </p>

            <nav className="flex flex-wrap justify-center gap-x-6 gap-y-2 font-serif italic text-sm text-[var(--color-abrow-muted)]">
              {enlaces.map((enlace) => (
                <a key={enlace.label} href={enlace.href} className="hover:text-[var(--color-abrow-dark)] transition-colors">
                  {enlace.label}
                </a>
              ))}
            </nav>
          ...
          <div className="space-y-3 md:text-right">
            <h5 className="font-serif text-lg text-[var(--color-abrow-dark)]">Contacto</h5>
